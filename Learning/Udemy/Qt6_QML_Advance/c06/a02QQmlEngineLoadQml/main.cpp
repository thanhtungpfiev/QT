...
int main(int argc, char *argv[])
{
    ...
    QList<QObject *> deepChildren
        = rootObject->findChildren<QObject *>("deepChild",
                                              Qt::FindChildrenRecursively); // Default Recursive
    if (deepChildren.count() > 0) {
        qDebug() << "Found " << deepChildren.count()
                 << " deepChildren, printing out their levels : ";
